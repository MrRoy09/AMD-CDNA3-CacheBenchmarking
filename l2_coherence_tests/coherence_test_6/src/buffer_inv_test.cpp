...
__global__ void buffer_inv_test_kernel(int *shared_value, int *results)
{
    int block_id = blockIdx.x;
    int tid = threadIdx.x;

    // Step 1: All XCDs load the value into their L2 cache
    ...
    // Step 2: Block 0 modifies the value
    if (block_id == 0 && tid == 0)
    {
        shared_value[0] = 42;
        asm volatile("s_waitcnt vmcnt(0)" ::: "memory");
    ...
    // Step 3: ALL threads across ALL XCDs issue buffer_inv sc1
    // This tests if buffer_inv sc1 invalidates L2 lines across all XCDs
    asm volatile("buffer_inv sc1" ::: "memory");
    asm volatile("s_waitcnt vmcnt(0)" ::: "memory");
    __syncthreads();

    // If buffer_inv worked, all threads should see coherent reads of stale value
    int new_value;
    for (int i = 0; i < 256; i++)
    {
        new_value = shared_value[0];
    }
...
int main(int argc, char **argv)
{
    ...
    std::cout << "Total CUs: " << props.multiProcessorCount << std::endl;

    // Launch enough workgroups to span multiple XCDs
    ...
    hipLaunchKernelGGL(buffer_inv_test_kernel,
                       dim3(num_blocks),
    ...
    std::cout << "42 read by: " << read_42 << " / 99 read by: " << read_99 << " / Wrong reads: " << wrong << std::endl;
