...
__global__ void wbl2_buffer_inv_load_kernel(int *shared_value, int *results)
{
    int block_id = blockIdx.x;
    int tid = threadIdx.x;

    // All workgroups read the old value to pull line into L2 and L1
    int old_value;
    for (int i = 0; i < 256; i++)
    {
    ...
    // Block 0, thread 0 performs regular store + wbl2
    if (block_id == 0 && tid == 0)
    {
        shared_value[0] = 42;
    ...
    // All threads perform regular load 256 times
    int new_value;
    for (int i = 0; i < 256; i++)
    {
        // All threads issue buffer_inv before loading
...
int main(int argc, char **argv)
{
    ...
    hipLaunchKernelGGL(wbl2_buffer_inv_load_kernel,
                       dim3(num_blocks),
