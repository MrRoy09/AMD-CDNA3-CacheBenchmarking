...
#define READ_ITERATIONS 200000

__global__ void load_nt_fixed_kernel(int *shared_value, int *results, int *xcc_ids)
{
    ...
    // All blocks do fixed NT loads with volatile asm
    if (tid == 0)
    {
        int value = 0;
...
int main(int argc, char **argv)
{
    ...
    hipLaunchKernelGGL(load_nt_fixed_kernel,
                       dim3(num_blocks),
    ...
    std::cout << "Fixed " << READ_ITERATIONS << " iterations NT load test (by XCC):\n";
    std::cout << "Checking if the final value read was 42\n\n";
    ...
    for (int xcc = 0; xcc < 8; xcc++)
    {
        ...
        if (block_count > 0)
        {
            std::cout << "XCC " << xcc << ": " << saw_42 << "/" << block_count << " blocks saw 42\n";
        }
    }
