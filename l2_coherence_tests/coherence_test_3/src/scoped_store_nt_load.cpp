...
__global__ void scoped_store_nt_load_kernel(int *shared_value, int *results)
{
    ...
    // Block 0, thread 0 performs device-scoped store (SC1)
    if (block_id == 0 && tid == 0)
    {
        int value = 42;
        asm volatile(
            ...
            :                          // No outputs
            : "v"(shared_value),       // %0 = address
              "v"(value)               // %1 = data to store
            : "memory");
        asm volatile("s_waitcnt vmcnt(0)" ::: "memory");
    }
    __syncthreads();
...
int main(int argc, char **argv)
{
    ...
    hipLaunchKernelGGL(scoped_store_nt_load_kernel,
                       dim3(num_blocks),
